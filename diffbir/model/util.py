...
def checkpoint(func, inputs, params, flag, importance_context=None):
    """
    ...
    """
    if flag:
        # ⭐ inputs에 importance_context 추가
        if importance_context is not None:
            inputs = tuple(inputs) + (importance_context,)
        else:
            inputs = tuple(inputs)
...
class CheckpointFunction(torch.autograd.Function):
    ...
    def backward(ctx, *output_grads):
        ...
        with torch.enable_grad(), \
            ...
            # Tensors.
            shallow_copies = [
                x.view_as(x) if x is not None else None 
