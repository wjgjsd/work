...
class SpacedSampler(Sampler):
    ...
    def apply_model(
        ...
        if uncond is None or cfg_scale == 1.0:
            model_output = model(x, model_t, cond, importance_map=importance_map)
        else:
            model_cond = model(x, model_t, cond, importance_map=importance_map)
            model_uncond = model(x, model_t, uncond, importance_map=importance_map)
    ...
    def p_sample(
        ...
        # predict x_0
        model_output = self.apply_model(model, x, model_t, cond, uncond, importance_map, cfg_scale)
    ...
    def sample(
        ...
        importance_map: torch.Tensor | None = None,#아마 여기다가 중요도 맵을 추가해야할 듯
        progress: bool = True,
    ) -> torch.Tensor:
