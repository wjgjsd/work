...
class Pipeline:
    ...
    def run(
        ...
        model_path='./importance_net_weights.pth'

        importance_net = SharedEncoderImportanceNet(in_channels=3)  # 6 → 3
        importance_net.load_state_dict(torch.load(model_path, map_location=device))
        importance_net.to(device)
        importance_net.eval()  # ← 평가 모드!

        with torch.no_grad():
            lr_upsampled = F.interpolate(
                lq_tensor, 
                size=cond_img.shape[2:],  # SR 크기에 맞춤
                mode='bicubic',
                align_corners=False
            )
            ...
            importance_map_tensor = importance_net(cond_img, lr_upsampled)  # ⭐ 텐서 유지!
            importance_map_np = importance_map_tensor.squeeze().cpu().numpy()  # 저장용

        # 🔥 파일 저장 (file_path가 제공되었을 때만)
        if file_path:  # 🔥 중요! self.file_path가 아닌 file_path
            ...
            # 파일명 추출 (확장자 제거)
            base_name = os.path.splitext(os.path.basename(file_path))[0]  # 🔥 self. 없음!
            
            # Importance map 저장
            import matplotlib.pyplot as plt
            imp_path = os.path.join(save_path, f'{base_name}_importance.png')
            plt.imsave(imp_path, importance_map_np, cmap='hot', vmin=0, vmax=1)
            print(f"✓ Importance map saved to {imp_path}")
            
            # 시각화
            fig, axes = plt.subplots(1, 3, figsize=(15, 5))
            
            # LR 이미지
            lr_np = lq_tensor.squeeze().permute(1, 2, 0).cpu().numpy()
            axes[0].imshow(lr_np)
            axes[0].set_title('LR Input Image')
            axes[0].axis('off')
            
            # Importance Map
            axes[1].imshow(importance_map_np, cmap='hot', vmin=0, vmax=1)
            axes[1].set_title('Predicted Importance Map')
            axes[1].axis('off')
            
            # Overlay
            sr_np = cond_img.squeeze().permute(1, 2, 0).cpu().numpy()
            axes[2].imshow(sr_np)
            axes[2].imshow(importance_map_np, cmap='hot', alpha=0.5, vmin=0, vmax=1)
            axes[2].set_title('Overlay on SR')
            axes[2].axis('off')
            
            plt.tight_layout()
            vis_path = os.path.join(save_path, f'{base_name}_visualization.png')
            plt.savefig(vis_path, dpi=150, bbox_inches='tight')
            print(f"✓ Visualization saved to {vis_path}")
            plt.close(fig)
        else:
            print("⚠️  Warning: file_path not provided, skipping importance map saving")
        ...
        sample = self.apply_cldm(
            ...
            eta,
            order,
            importance_map=importance_map_tensor,
        )
