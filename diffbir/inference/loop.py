...
class InferenceLoop:
    ...
    def load_lq(self) -> Generator[Image.Image, None, None]:
        ...
        for file_name in sorted(os.listdir(self.args.input)):
            ...
            self.loop_ctx["file_path"] = file_path  # 🔥 추가!
            self.loop_ctx["file_ext"] = ext         # 🔥 추가 (옵션)
            yield lq
    ...
    def run(self) -> None:
        ...
        for lq in self.load_lq():
            ...
            # 🔥 파일 경로 가져오기 (loop_ctx에서)
            file_path = self.loop_ctx.get("file_path", None)
            ...
            for i in range(num_batches):
                ...
                with torch.autocast(self.args.device, auto_cast_type):
                    batch_samples = self.pipeline.run(
                        ...
                        self.args.eta,
                        self.args.order,
                        file_path=file_path if type(file_path) is str else None,
                    )
